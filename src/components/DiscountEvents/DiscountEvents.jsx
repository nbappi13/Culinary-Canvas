...
const DiscountEvents = () => {
  ...
  useEffect(() => {
    const fetchDiscountFoods = async () => {
      try {
        setLoading(true)
        const response = await axios.get("http://localhost:5000/api/discounts/foods")
    ...
    return (
      <section className="py-16 mt-10 bg-gradient-to-b from-amber-50 to-orange-50 dark:from-gray-800 dark:to-gray-900 px-4">
        <div className="container mx-auto text-center">
          <h2 className="text-4xl font-bold mb-8 text-amber-800 dark:text-amber-400">🔥 Limited Time Offers</h2>
    ...
    <section className="py-16 mt-10 bg-gradient-to-b from-amber-50 to-orange-50 dark:from-gray-800 dark:to-gray-900 px-4">
      <div className="container mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-amber-800 dark:text-amber-400 mb-3">🔥 Limited Time Offers</h2>
          <p className="text-gray-700 dark:text-gray-300 max-w-2xl mx-auto">
            Enjoy special discounts on our oldest menu items. These classic favorites come with amazing savings!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {foods.length > 0 ? (
            foods.map((food) => {
              ...
              const discountedPrice = food.discountedPrice || (food.price * 0.7).toFixed(2)
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    <h3 className="font-bold text-xl text-gray-800 dark:text-white mb-2">{food.name}</h3>
                    <div className="flex items-center justify-between mb-4">
                      <span className="line-through text-gray-500 dark:text-gray-400">${food.price.toFixed(2)}</span>
            ...
            <div className="col-span-full bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-300 text-lg">No discount items available at the moment.</p>
              <p className="text-gray-500 dark:text-gray-400 mt-2">Please check back later for special offers!</p>
            </div>
          )}
        </div>
      </div>
